...
export class ImportExportService {
  ...
  async importData(fileBuffer: Buffer, fileName: string, options: ImportOptions): Promise<ImportResult> {
    ...
    try {
      ...
      if (options.format === 'csv' || fileExtension === '.csv') {
        const fileContent = fileBuffer.toString(options.encoding || 'utf8');
  ...
  private async importSingleType(
    ...
    const existingBudgets = options.type === 'budgets' ? await this.budgetRepo.findAll() : [];
      ...
      try {
        switch (options.type) {
          ...
          case 'categories':
            ...
            break;
          case 'budgets':
            isValid = this.validator.validateBudgetRow(row, i + 1, existingCategories, existingBudgets.data || []);
            break;
        }
