...
import { getDateRange, getTrend, computePreviousPeriodDates } from './analytics/analyticsHelpers';
...
export class AnalyticsService {
  ...
  async getBudgetPerformance(query: AnalyticsQuery = {}): Promise<BudgetPerformance> {
    try {
      ...
      const budgetsOnTrack = activeBudgets.filter(b => b.progress_percentage <= 80 && !b.is_overspent).length;
      const budgetsApproachingLimit = activeBudgets.filter(b => b.progress_percentage > 80 && b.progress_percentage <= 100).length;
  ...
  async getTrendsAnalysis(query: AnalyticsQuery = {}): Promise<TrendsAnalysis> {
    try {
      ...
      return {
        period: { start_date, end_date, granularity },
        ...
        net_worth_trends: insights.spending_trends.map((trend, index) => {
          const previousCumulative = index > 0
            ? insights.spending_trends.slice(0, index).reduce((sum, t) => sum + t.net_amount, 0)
            : 0;
  ...
  async getComparisonAnalysis(query: AnalyticsQuery = {}): Promise<ComparisonAnalysis> {
    try {
      const { start_date, end_date } = getDateRange(query);
      const { previousStartDate, previousEndDate } = computePreviousPeriodDates(start_date, end_date);

      const [currentInsights, previousInsights] = await Promise.all([
        this.getSpendingInsights({ start_date, end_date }),
        this.getSpendingInsights({ start_date: previousStartDate, end_date: previousEndDate })
      ]);
