...
export class AnalyticsService {
  ...
  async getComparisonAnalysis(query: AnalyticsQuery = {}): Promise<ComparisonAnalysis> {
    try {
      ...
      const previousEndDate = new Date(new Date(start_date).getTime() - 24 * 60 * 60 * 1000).toISOString().split('T')[0];
      const previousStartDate = new Date(new Date(start_date).getTime() - periodLength - 24 * 60 * 60 * 1000).toISOString().split('T')[0];
